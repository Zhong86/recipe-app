...
<div class="recipe-form-page">
    ...
    <form
        action="{{ isset($recipe) ? url('/recipes/'.$recipe->id.'/update') : url('/recipes/create') }}"
        method="POST"
    ...
    <div class="recipe-form-grid">
        ...
        <div class="recipe-form-main">
            ...
            <div class="form-block">
                ...
                <div class="form-row">
                    <div class="form-field" style="margin-bottom:0">
                        ...
                        >
                            <option value="" disabled {{ old('category', $recipe->category ?? '') === '' ? 'selected' : '' }}>Select category…</option>
                ...
                <div id="upload-area">
                    @if(isset($recipe) && $recipe->image_url)
                        <div class="image-preview" id="img-preview">
                            ...
                            <button type="button" class="image-preview-remove" id="remove-img" title="Remove image">✕</button>
                        </div>
                    ...
                    @else
                        ...
                        <div class="image-preview" id="img-preview" style="display:none;margin-top:.75rem">
                            ...
                            <button type="button" class="image-preview-remove" id="remove-img" title="Remove">✕</button>
                        </div>
                    @endif
                </div>
                @error('image')
            ...
            <div class="form-block">
                ...
                <table class="ingredients-table">
                    ...
                    <tbody id="ingredients-body">
                        @php
                            $existingIngredients = old('ingredients', isset($recipe) ? $recipe->ingredients->toArray() : [[]]);
                            if (empty($existingIngredients)) $existingIngredients = [[]];
                        @endphp
                        ...
                        <tr class="ingredient-row">
                            ...
                            <td>
                                <select name="ingredients[{{ $i }}][unit]" class="form-select">
                                    @php
                                        $units = ['tsp','tbsp','cup','ml','l','g','kg','oz','lb','piece','slice','pinch','bunch'];
                                        $selectedUnit = isset($ingredient['unit']) ? (is_object($ingredient['unit']) ? $ingredient['unit']->value : $ingredient['unit']) : '';
                                    @endphp
            ...
            <div class="form-block">
                ...
                <div id="steps-container">
                    @php
                        $existingSteps = old('steps', isset($recipe)
                            ? $recipe->steps->sortBy('order')->pluck('instruction')->toArray()
                            : ['']);
                        if (empty($existingSteps)) $existingSteps = [''];
                    @endphp
                    ...
                    <div class="step-entry" data-index="{{ $s }}">
                        ...
                        <textarea
                            ...
                            rows="3"
                        >{{ $step }}</textarea>
                        <button type="button" class="remove-row-btn" onclick="removeStep(this)" title="Remove step" style="margin-top:.55rem">✕</button>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="form-block">
                ...
                <div class="form-field">
                    <div class="tags-wrap" id="tags-wrap" onclick="document.getElementById('tag-input').focus()">
                        @php
                            $existingTags = old('tags', isset($recipe) ? $recipe->tags->pluck('name')->toArray() : []);
                        @endphp
                        @foreach($existingTags as $tag)
                            <span class="tag-chip" data-tag="{{ $tag }}">
                                {{ $tag }}
                                <input type="hidden" name="tags[]" value="{{ $tag }}">
                                <button type="button" onclick="removeTag(this.parentElement)" aria-label="Remove tag">✕</button>
                            </span>
                        @endforeach
                        <input
                            type="text"
                    ...
                    <p class="form-hint">e.g. <em>vegan, quick, comfort food</em> — press <kbd style="font-family:monospace;font-size:.75rem;background:var(--parchment);padding:.1rem .35rem;border-radius:2px;border:1px solid var(--border)">Enter</kbd> or comma to add</p>
                </div>
            </div>
        ...
        <aside class="form-sidebar">
            ...
            <div class="sidebar-publish">
                ...
                <a
                    href="{{ isset($recipe) ? url('/recipe/'.$recipe->id) : url('/recipes') }}"
                    class="btn-cancel"
                >
                    Cancel
                </a>
...
function addTag() {
    ...
    if (!val || existing.includes(val.toLowerCase())) { tagInput.value = ''; return; }
...
function removeTag(chip) { chip.remove(); }
...
function updateChecklist() {
    ...
    setCheck('check-ingredients', [...document.querySelectorAll('#ingredients-body input[type=text]')].some(i => i.value.trim()));
